        ...
        for line in lines:
            ...
            if mode!='testing':
                for event in doc['events']:
                    sorted_mentions = sorted(event['mention'], key=lambda x: (x['sent_id'], x['offset'][0]))
                    prev_sent_id = None
                    prev_end_offset = None
                    prev_type = None
                    for mention in sorted_mentions:
                        sent_id = mention['sent_id']
                        start_offset = mention['offset'][0]
                        end_offset = mention['offset'][1]

                        if prev_sent_id == sent_id and prev_end_offset == start_offset and prev_type == event['type']:
                            labels[sent_id][start_offset] = "I-" + event['type']
                        else:
                            labels[sent_id][start_offset] = "B-" + event['type']

                        for i in range(start_offset + 1, end_offset):
                            labels[sent_id][i] = "I-" + event['type']
                ...
                # Change consecutive B- tags of the same event type to I- tags
                for label_list in labels:
                    i = 1
                    while i < len(label_list):
                        if label_list[i].startswith("B-") and label_list[i - 1].startswith("B-") and label_list[i][2:] == label_list[i - 1][2:]:
                            label_list[i] = "I-" + label_list[i][2:]
                            j = i + 1
                            while j < len(label_list) and label_list[j].startswith("B-") and label_list[j][2:] == label_list[i][2:]:
                                label_list[j] = "I-" + label_list[j][2:]
                                j += 1
                        i += 1
...
def get_labels(path):
    ...
    # return ["O", "B-Binding", "I-Binding", "B-Gene_expression", "I-Gene_expression", "B-Localization", "I-Localization", "B-Negative_regulation", "I-Negative_regulation", "B-Phosphorylation", "I-Phosphorylation", "B-Positive_regulation", "I-Positive_regulation", "B-Protein_catabolism", "I-Protein_catabolism", "B-Regulation", "I-Regulation", "B-Transcription", "I-Transcription"]
    return ["O", "B-Databreach", "I-Databreach", "B-Phishing", "I-Phishing", "B-Ransom", "I-Ransom", "B-PatchVulnerability", "I-PatchVulnerability", "B-DiscoverVulnerability", "I-DiscoverVulnerability"]
    # labels = []
    # with open('../maven/ge11/labels.json', 'r') as openfile:
